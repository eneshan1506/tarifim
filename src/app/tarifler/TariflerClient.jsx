...
export default function TariflerClient({ categories, recipes, initialSavedRecipeIds }) {
  ...
  if (activeCategory !== "tumu") filtered = filtered.filter((r) => r.category === activeCategory);
  if (activeDifficulty !== "Tümü") filtered = filtered.filter((r) => r.difficulty === activeDifficulty);
  if (search) filtered = filtered.filter((r) => r.title.toLowerCase().includes(search.toLowerCase()) || r.desc.toLowerCase().includes(search.toLowerCase()));
  if (sortBy === "hizli") filtered.sort((a, b) => a.time - b.time);
  else if (sortBy === "begeni") filtered.sort((a, b) => b.likes - a.likes);
  else if (sortBy === "yeni") filtered.sort((a, b) => new Date(b.publishedAt || b.createdAt) - new Date(a.publishedAt || a.createdAt));
  else filtered.sort((a, b) => (b.likes + b.comments * 3) - (a.likes + a.comments * 3));
  ...
  return (
    <SiteLayout>
      ...
      {/* Filtre bar */}
      <section className="mb-6 rounded-2xl bg-white p-4 shadow-[0_12px_28px_rgba(126,74,38,0.08)] ring-1 ring-[#f4e8db]/70 animate-fade-in-up delay-1">
        {/* Arama */}
        <div className="search-box mb-4 flex items-center gap-2 rounded-full bg-[#fffdf8] px-4 py-2.5 ring-1 ring-[#f1dac3]/60">
      ...
      <section className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filtered.map((recipe, i) => (
          <article
            key={recipe.id}
            className="recipe-card group overflow-hidden rounded-2xl bg-white shadow-[0_12px_28px_rgba(126,74,38,0.08)] ring-1 ring-[#f4e8db]/70 animate-fade-in-up"
          ...
          >
              ...
              <div className="relative h-48 overflow-hidden bg-gradient-to-br from-[#ffe8cc] to-[#ffd6a5]">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 flex items-end justify-between z-10">
                  ...
                  <button
                    ...
                    disabled={isSaving}
                    className="flex h-8 w-8 items-center justify-center rounded-full bg-black/30 text-white backdrop-blur transition-colors hover:bg-black/50 disabled:opacity-60"
            ...
            <div className="p-4">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <span className="text-xs font-semibold text-[#b89a7c]">❤️ {recipe.likes} favori</span>
              </div>
            </div>
          </article>
        ))}
